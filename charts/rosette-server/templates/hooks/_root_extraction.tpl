...
- name: {{ .Release.Name }}-populate-rli
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rli:" .Values.roots.version.rli | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
- name: {{ .Release.Name }}-populate-rbl
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rbl:" .Values.roots.version.rbl | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
    {{- $installedRoots := list -}}
...
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rex-root:" $.Values.roots.version.rex | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
...
- name: {{ $.Release.Name }}-populate-rex-{{$lang}}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rex-" $lang ":" $.Values.roots.version.rex | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
                    {{- end }}
        ...
        {{- if eq $ep "categories" }}
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-tcat:" $.Values.roots.version.tcat | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
        {{- end }}
...
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rni-rnt:" $.Values.roots.version.rnirnt | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
        ...
        {{- if eq $ep "sentiment" }}
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-ascent:" $.Values.roots.version.ascent | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
        {{- end }}
...
- name: {{ print $.Release.Name "-populate-" "nlp4j" | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-nlp4j:" $.Values.roots.version.nlp4j | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
...
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-rct:" $.Values.roots.version.rct | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
        {{- end }}
...
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-relax:" $.Values.roots.version.relax | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
        {{- end }}
...
- name: {{ print $.Release.Name "-populate-" $ep | quote }}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-topics:" $.Values.roots.version.topics | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
...
- name: {{ $.Release.Name }}-populate-vec-{{$lang}}
  image: {{ print (include "rosette-server.image.registry" .) $.Values.rootsImageRepository "root-tvec-" $lang ":" $.Values.roots.version.tvec | quote }}
  volumeMounts:
    - mountPath: "/roots-vol"
      name: {{ $.rootsVolumeName }}
                    {{- end }}
