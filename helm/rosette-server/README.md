...
The configuration can be deployed by running [`stage-rosette-servers.sh`](/rosette-server/stage-rosette-server.sh). It will create the configuration directories in `helm/helm/rosette-server`. Note: the configuration files must be deployed inside the chart directory (`helm/helm/rosette-server`) if the configuration files were installed in a subdirectory then the `values.yaml` must be updated and the `confDirectory`, `configDirectory` and `configRosapiDirectory` values updated to point the subdirectory. This Chart assumes that the `rosette/server-enterprise:1.27.0` image is being used. If a different version is being used please update `Chart.yaml` and update the `appVersion` Please refer to the README file in the `helm/rosette-server` directory for more information.
